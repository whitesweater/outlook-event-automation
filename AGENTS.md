...
## Local Notes
- This repo contains a small production-oriented Outlook/Gmail mail-to-calendar service plus a static GitHub Pages site. Keep changes scoped and easy to audit.
- The runtime service lives in `outlook_event_automation/event_agent.py` and intentionally uses the Python standard library only. Do not add dependencies unless the user explicitly asks.
